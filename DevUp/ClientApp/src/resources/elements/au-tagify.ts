...
export class AuTagify {
    ...
    setupTags(): void {
        ...
        this.tagify = new (Tagify as any)(this.inputEl, {
            ...
            templates: {
                ...
                dropdownItem: function (tagData) {
                    let background = tagData.backGroundColorHex ? `background:${tagData.backGroundColorHex};` : '';
                    let color = tagData.textColorHex ? `color:${tagData.textColorHex};` : '';
                    return `
        ...
        const tagsToAdd = this.tagify?.settings?.whitelist?.filter(x => valueIds.includes(x.name)) ?? [];
